...
{#if music}
  <main>
    ...
    <Canvas size={{ height: window.innerHeight, width: window.innerWidth }}>
      <Fog color={"#444444"} near={20} far={100} />
      <HTML zIndexRange={[0, 1]} occlude transform distanceFactor={1.17} position={{ x: -19.6, y: 28, z: -8.25 }} rotation={{ y: -Math.PI / 2 }}>
        <img
          src="./invis.gif"
          alt=""
          on:click={() => {
      ...
      <Group scale={$scale}>
        ...
        {#if music}
          ...
          <Audio source={"/crowd.mp3"} loop autoplay playbackRate="1" volume={0.1} />

          <Audio source={"/bg.mp3"} loop autoplay playbackRate="1" volume={0.7} />
        {/if}
      </Group>
    </Canvas>
  </main>
